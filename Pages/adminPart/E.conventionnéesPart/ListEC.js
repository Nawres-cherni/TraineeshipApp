...
const ListEC = ({props,navigation}) => {
  ...
  useEffect(() => {
      firebase.firestore().collection("e_conventionnées").onSnapshot((querySnapshot) => {
      
          const users = [];
         
          querySnapshot.docs.forEach((doc) => {
      
            const {nom_soc,nom_res, email, fax, fix,imageUrl,site ,location} = doc.data();
            users.push({
              id: doc.id,
              nom_soc,
              nom_res,
...
//source={user.imageUrl && {uri: user.imageUrl }}
           ...
           size={80}
              ...
              <Text>{user.nom_soc}</Text>
              <Text>{user.nom_res}</Text>
    ...
    <View style={{marginBottom:90}}/>
        ...
        <ActionButton buttonColor="#1f487e"    style={{marginBottom:70}} 
        >
      <ActionButton.Item
        buttonColor="#9b59b6"
        title="Ajouter Entreprise"
        onPress={() => {
          navigation.navigate("createEC")
        }}
        onScroll={Animated.event(
