...
export default class MessageEc extends Component {
    ...
    getMessages = () => {
        ...
        let data = []
        firebase.firestore().collection('e_conventionnées').doc(firebase.auth().currentUser.uid).onSnapshot((snap) => {
    ...
    navigateChatScreen = (data) => {
        ...
        if (found) {
            ...
            this.props.navigation.push('ChatEC', { name: data.name, imageUrl: data.imageUrl, uid: data.uid, chatId: chatId, blocked: blocked, blockId: blockId, typing: [], bgColor: this.state.bgColor })
        }
        else {
            console.log('not found')
            this.props.navigation.push('ChatEC', { name: data.name, imageUrl: data.imageUrl, uid: data.uid, blocked: false, blockId: [], typing: [], bgColor: this.state.bgColor })
        }
...
<Text style={styles.headerTitle}>Message</Text>
                        ...
                        {this.state.chatsData.map((data) => (
                            <TouchableRipple
                                onPress={() => this.props.navigation.push('ChatEC', { name: data.name, uid: data.uid, imageUrl: data.imageUrl, chatId: data.chatId, blocked: data.blocked, blockId: data.blockId, typing: data.typing, bgColor: this.state.bgColor })}
                                rippleColor="rgba(0, 0, 0, .32)"
                                borderless={true}
                            >
                                <UserListItem
