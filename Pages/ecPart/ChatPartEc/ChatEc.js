...
export default class ChatEc extends Component {
    ...
    getChats = async () => {
        firebase.firestore().collection('chats').doc(this.state.chatId).onSnapshot((snap) => {
    ...
    textInputChange = (val) => {
        ...
        if (val.length > 0) {
            firebase.firestore().collection('chats').doc(this.state.chatId).update({
                typing: firebase.firestore.FieldValue.arrayUnion(firebase.auth().currentUser.uid)
            })
        }
        else {
            this.removeTypingId()
    ...
    removeTypingId = () => {
        firebase.firestore().collection('chats').doc(this.state.chatId).update({
            typing: firebase.firestore.FieldValue.arrayRemove(firebase.auth().currentUser.uid)
        })
    }
    ...
    updateSeen = () => {
        ...
        messages.map((message) => {
            if (!message.seen && message.uid !== firebase.auth().currentUser.uid) {
                message['seen'] = true
                seenfound = true
            }
        })
    ...
    setNewChatId = (newMessage) => {
        ...
        this.setState({ chatId: id }, () => {
            ...
            firebase.firestore().collection('chats').doc(id).set({
                messages: [newMessage],
                blocked: false,
                blockId: [],
                typing: []
            })
            .then(() => firebase.firestore().collection('e_conventionnées').doc(this.props.route.params.uid).update({
    ...
    blockUser = () => {
        firebase.firestore().collection('chats').doc(this.state.chatId).update({
            blocked: true,
            blockId: firebase.firestore.FieldValue.arrayUnion(firebase.auth().currentUser.uid)
        }).then(() => {
            let ar = this.state.blockId
            ar.push(firebase.auth().currentUser.uid)
    ...
    unbockUser = () => {
        ...
        if (this.state.blockId.length === 1 && this.state.blockId.includes(firebase.auth().currentUser.uid)) {
            blocked = false
        }
        firebase.firestore().collection('chats').doc(this.state.chatId).update({
            blocked: blocked,
            blockId: firebase.firestore.FieldValue.arrayRemove(firebase.auth().currentUser.uid)
        }).then(() => {
            let ar = this.state.blockId
            ar.splice(ar.indexOf(firebase.auth().currentUser.uid), 1)
    ...
    saveColor = (color) => {
        ...
        this.RBSheet.close()
        firebase.firestore().collection('e_conventionnées').doc(firebase.auth().currentUser.uid).update({
            bgColor: color
        }).then(() => {
            this.setState({
                saveColorLoading: false,
                ...
                <View style={{ flex: 1 }} >
                    ...
                    <View style={styles.header} >
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }} >
                            <Avatar.Image size={40} source={{ uri: this.props.route.params.imageUrl }} />
                            <View>
                    ...
                    <ScrollView style={{ flex: 1 }} >
                        {this.state.messages.map((message) => (
                            <ChatMessage
                                ...
                                seen={message.seen}
                                me={firebase.auth().currentUser.uid === message.uid}
                    ...
                    >
                        {this.state.blockId.includes(firebase.auth().currentUser.uid) ?
                            <TouchableRipple
